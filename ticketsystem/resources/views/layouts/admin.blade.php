<html>
    ...
    <body class="bg-gray-600">
        <header class="flex h-28 items-center bg-gray-800 text-gray-300 text-xl">
            <a class="text-3xl m-5 font-bold hover:text-white transition duration-300" href="/">Ticketsysteem</a>
            <nav class="flex w-2/5 justify-center gap-10">
                <a class="hover:text-white" href="{{ route('admin.user') }}">Users</a>
                <a class="hover:text-white" href="{{ route('admin.event') }}">Events</a>
            ...
            @if (Route::has('login'))
                <div class="flex w-3/5  gap-5 justify-end">
                    @auth
                        <div class="hidden sm:flex sm:items-center sm:ml-6">
                            ...
                            <div x-show="open" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 scale-95" x-transition:enter-end="opacity-100 scale-100" x-transition:leave="transition ease-in duration-75" x-transition:leave-start="opacity-100 scale-100" x-transition:leave-end="opacity-0 scale-95" class="absolute z-50 mt-2 w-48 rounded-md shadow-lg origin-top-right right-0" @click="open = false" style="display: none;">
                                <div class="rounded-md ring-1 ring-black ring-opacity-5 py-1 bg-white dark:bg-gray-700">
                                    <a class="block w-full px-4 py-2 text-left text-sm leading-5 text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800 focus:outline-none focus:bg-gray-100 dark:focus:bg-gray-800 transition duration-150 ease-in-out" href="http://127.0.0.1:8000/profile">{{ auth()->user()->name }}</a>
                                    <a class="block w-full px-4 py-2 text-left text-sm leading-5 text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800 focus:outline-none focus:bg-gray-100 dark:focus:bg-gray-800 transition duration-150 ease-in-out" href="/Reservations">Reservations</a>s

                                    <!-- Authentication -->
                                    <form method="POST" action="http://127.0.0.1:8000/logout">
                                        <input type="hidden" name="_token" value="{{ csrf_token() }}" autocomplete="off">
                                        <a class="block w-full px-4 py-2 text-left text-sm leading-5 text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800 focus:outline-none focus:bg-gray-100 dark:focus:bg-gray-800 transition duration-150 ease-in-out" href="http://127.0.0.1:8000/logout" onclick="event.preventDefault(); this.closest('form').submit();">Uitloggen</a>
                                    </form>
                                </div>
                            </div>
                        </div>
                    ...
                    @else
                    <div>
                        <a href="{{ route('login') }}" class="text-gray-300 hover:text-white transition duration-300 focus:outline focus:outline-2 focus:rounded-sm focus:outline-red-500">Inloggen</a> |
                        @if (Route::has('register'))
                            <a href="{{ route('register') }}" class="text-gray-300 hover:text-white transition duration-300 focus:outline focus:outline-2 focus:rounded-sm focus:outline-red-500">Registreren</a>
                        @endif
                    </div>
                    @endauth
                </div>
